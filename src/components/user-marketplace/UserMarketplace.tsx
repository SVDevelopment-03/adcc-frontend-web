...
function MarketplaceCard({ item }: { item: StoreItem }) {
  ...
  return (
    <div
      role="button"
      tabIndex={0}
      onClick={() => id && navigate(`/user-marketplace/${id}`)}
      onKeyDown={(event) => {
...
export default function UserMarketplace() {
  ...
  const heroBreadcrumb = useWordList(
    "public.marketplace.listing.hero.breadcrumb",
  );
  const browseTitleWords = useWordList(
    "public.marketplace.listing.browseTitleWords",
  );
  const gearUpTitleWords = useWordList(
    "public.marketplace.listing.gearUp.titleWords",
  );

  const [items, setItems] = useState<StoreItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [page, setPage] = useState(1);
  const [pagination, setPagination] = useState<StoreItemsPagination>({
    page: 1,
  ...
  useEffect(() => {
    getStoreItemsPage({ status: "Approved", page: 1, limit: 100 })
      .then(({ items }) => {
    ...
    <div className="min-h-screen bg-[#eaf4ff] text-black">
      ...
      <section className="w-full px-4 py-10 sm:px-6 sm:py-20 md:px-10 lg:px-20 lg:py-24">
        ...
        <div className="mt-6 grid grid-cols-1 gap-4 sm:mt-10 sm:gap-5 lg:grid-cols-[1fr_292px_156px]">
          <div className="flex h-[52px] items-center rounded-full border border-[#CBCBCB] bg-white px-6 sm:h-[66px] sm:px-8">
            <input
              ...
              placeholder={t("public.marketplace.listing.searchPlaceholder")}
          ...
          <div className="relative">
            ...
            >
              <span className="truncate">
                {selectedCategory ||
                  t("public.marketplace.listing.allCategories")}
              </span>
              <ChevronDown
                size={20}
              ...
              <div className="absolute inset-x-0 top-full z-20 mt-2 max-h-[280px] overflow-y-auto rounded-2xl border border-[#CBCBCB] bg-white p-2 shadow-lg">
                <button
                  ...
                  className={`block w-full rounded-xl px-4 py-2.5 text-start text-[15px] hover:bg-[#eaf4ff] ${
                    !selectedCategory ? "font-bold text-[#019839]" : ""
                  }`}
                >
                  {t("public.marketplace.listing.allCategories")}
                </button>
        ...
        {!loading && pagination.pages > 1 && (
          <div className="mt-10 flex flex-wrap items-center justify-center gap-2 text-[#019839] text-[13px] font-medium sm:mt-20 sm:gap-3 sm:text-[16px] lg:gap-8 lg:text-[20px]">
            <button
              type="button"
      ...
      <section className="pt-8 relative grid w-full grid-cols-1 items-end gap-8 overflow-hidden px-4 pb-12 sm:gap-16 sm:px-6 sm:pb-28 md:px-10 lg:grid-cols-2 lg:px-20">
        <div>
          ...
          <p className="mt-6 max-w-[615px] text-[15px] font-normal leading-6 sm:mt-12 sm:text-[24px] sm:leading-[30px]">
            {t("public.marketplace.listing.gearUp.body")}
          </p>
        ...
        <div className="grid grid-cols-1 gap-16 md:grid-cols-3">
          ...
          <div>
            ...
            <ul className="mt-8 space-y-4 text-[18px]">
              ...
              <li className="flex gap-3">
                <Mail size={22} />{" "}
                {t("public.marketplace.listing.footer.address")}
              </li>
              <li className="flex gap-3">
