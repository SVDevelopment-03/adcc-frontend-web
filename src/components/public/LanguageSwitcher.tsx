    ...
    <div ref={rootRef} className={cn('relative', className ?? 'hidden md:block')}>
      ...
      {open && (
        <ul
          ...
          className={`absolute top-full z-[130] mt-2 min-w-[140px] overflow-hidden rounded-xl border border-black/10 bg-white py-1 shadow-[0_12px_32px_rgba(0,0,0,0.14)] ${
          ...
          {LANGUAGES.map(({ code, labelKey }) => (
            <li key={code} role="option" aria-selected={locale === code}>
              <button
                ...
                className={`flex w-full items-center px-4 py-2.5 text-[15px] transition-colors hover:bg-[#019839]/10 ${
                  locale === code
