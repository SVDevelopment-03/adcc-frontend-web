...
export const sendStaffWebPush = async (
...
// Send test/broadcast push notification (supports audienceType: staff|all)
export const sendTestBroadcastPush = async (
  payload: PushNotificationPayload
): Promise<SendStaffWebPushResponse> => {
  try {
    const formData = new FormData();
    ...
    const response = await api.post<SendStaffWebPushResponse>(
      '/v1/push-notifications/test-broadcast',
      formData
    );
    return response.data;
  } catch (error) {
    console.error('Error sending test broadcast push:', error);
    throw error;
  }
};
