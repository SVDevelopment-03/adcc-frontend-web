...
export const trackCommunityResults = async (
  ...
  try {
    ...
    if (body?.data && typeof body.data === "object") {
      const dataObj = body.data as { communities?: any[] };
      if (Array.isArray(dataObj.communities)) return dataObj.communities;
    }

    if (Array.isArray(body?.communities)) return body.communities;
